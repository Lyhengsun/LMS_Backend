...
public class UserManagementController {
    ...
    public ResponseEntity<ApiResponse<PagedResponse<AppUserResponse>>> getAllUser(
        ...
        return ResponseUtils.createResponse("Get all users", userManagementService.getAllUser(page, size, isApproved));
    }

    @GetMapping("{user-id}")
    public ResponseEntity<ApiResponse<AppUserResponse>> getUserById(@PathVariable("user-id") Long id) {
        return ResponseUtils.createResponse("Get user with id " + id + " successful", userManagementService.getUserById(id));
    }
    ...
    public ResponseEntity<ApiResponse<AppUserResponse>> updateUser(@PathVariable("user-id") Long id, @RequestBody AppUserRequest appUserRequest) {
        return ResponseUtils.createResponse("Update user successfully", userManagementService.updateUser(id, appUserRequest));
    }

    @PatchMapping("update-user/{user-id}/disable")
    public ResponseEntity<ApiResponse<Object>> updateDisableUser(@PathVariable("user-id") Long id, @RequestParam(defaultValue = "false") @NotNull Boolean isDisable) {
